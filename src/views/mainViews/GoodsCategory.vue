...
import { category } from "../../util/clazz"
...
const updateCategory = () => {
    axios.post('/goods/updateGoodsCategory' , form).then(response => {
...
const addCategory = () => {
    axios.post('/goods/addGoodsCategory' , form).then(response => {
...
const search = () => {
    //   pageNum = 1
    axios.get('/goods/listGoodsCategory/' + pageNum.value + '/14', {
        params: {
            'keyword': input1.value,
        }
    }).then(response => {
        if (response.data.code == 200) {
            data.data = response.data.data.list as category[]
...
const deleteCategory = (index, row) => {
    ElMessageBox.confirm(
        ...
        .then(() => {
            ...
            axios.get('/goods/deleteGoodsCategory/' + row.id, {
                params: {
                }
            }).then(response => {
                if (response.data.code == 200 && response.data.data != -1) {
