<template>
    ...
    <el-table :data="data.data" style="width: 95%;height: 80%;">
        ...
        <el-table-column prop="userId" label="卖家id" width="200" />
...
import { Goods, category } from "../../util/clazz"
...
const queryGoods = (scope, row) => {
    axios.get('/goods/queryGoodsById/' + row.id, {
    }).then(response => {
        if (response.data.code == 200) {
...
const search = () => {
    //   pageNum = 1
    axios.get('/goods/adminList/' + pageNum.value + '/4', {
        params: {
        ...
        if (response.data.code == 200) {
            data.data = response.data.data.list as Goods[]
...
const deleteOrder = (index, row) => {
    ElMessageBox.confirm(
        ...
        .then(() => {
            ...
            axios.get('/goods/adminDelete/' + row.id, {
                params: {
                }
            }).then(response => {
